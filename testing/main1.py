...
if selected == "Attendance System":
    ...
    def capture_and_save_attendance(cap, known_faces, known_names):
        ...
        while cap.isOpened() and not stop_button_pressed:
            ...
            # Draw rectangles around detected faces and process attendance
            for (top, right, bottom, left), face_encoding in zip(face_locations, face_encodings):
                ...
                if True in matches:
                    ...
                    if last_attendance_time is None or (current_time - datetime.timedelta(minutes=5)) > last_attendance_time:
            ...
            st.empty().image(frame, channels="RGB")
    ...
    #                 first_match_index = matches.index(True)
    #                 name = known_names[first_match_index]
    #                 attendance_data.append([name, datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")])
    #                 print(name, datetime.datetime.now())
    #
    #             cv2.rectangle(frame, (left, top), (right, bottom), (0, 0, 255), 2)  # Red color (BGR)
    #             cv2.rectangle(frame, (left, bottom - 35), (right, bottom), (0, 0, 255), cv2.FILLED)
    ...
    #     cv2.destroyAllWindows()
    if selected == "Using Webcam":
        # Streamlit app code (omitted for brevity)
        cap = cv2.VideoCapture(0)
        # Button to stop the app
        stop_button_pressed = st.button("Stop")
        capture_and_save_attendance(cap, known_faces, known_names)
